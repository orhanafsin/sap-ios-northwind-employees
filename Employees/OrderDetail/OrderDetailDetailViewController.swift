//
// OrderDetailDetailViewController.swift
// Employees
//
...
class OrderDetailDetailViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private let logger = Logger.shared(named: "OrderDetailMasterViewControllerLogger")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "updateEntity" {
            ...
            let detailViewController = dest.viewControllers[0] as! OrderDetailDetailViewController
    ...
    override func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 5:
            ...
            let destinationDetailVC = destinationStoryBoard.instantiateViewController(withIdentifier: "OrderDetailViewController") as! OrderDetailViewController
            self.northwindEntities.loadProperty(OrderDetail.order, into: self.entity) { error in
                ...
                if let order = self.entity.order {
                    destinationDetailVC.entity = order
                }
                destinationDetailVC.navigationItem.leftItemsSupplementBackButton = true
                destinationDetailVC.navigationItem.title = "Order"
        ...
        case 6:
            ...
            self.northwindEntities.loadProperty(OrderDetail.product, into: self.entity) { error in
                self.hideFioriLoadingIndicator()
                if let error = error {
                    let alertController = UIAlertController(title: NSLocalizedString("keyErrorLoadingData", value: "Loading data failed!", comment: "XTIT: Title of loading data error pop up."), message: error.localizedDescription, preferredStyle: .alert)
                    alertController.addAction(UIAlertAction(title: self.okTitle, style: .default))
                ...
                if let product = self.entity.product {
                    destinationDetailVC.entity = product
                }
                destinationDetailVC.navigationItem.leftItemsSupplementBackButton = true
...
extension OrderDetailDetailViewController: EntityUpdaterDelegate {
