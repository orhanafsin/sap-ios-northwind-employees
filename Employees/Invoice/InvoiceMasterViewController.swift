//
// InvoiceMasterViewController.swift
// Employees
//
...
class InvoiceMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([Invoice]?, Error?) -> Void) -> Void)?
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CustomerName, Discount, OrderID, ProductID, ProductName, Quantity, Salesperson, ShipperName, UnitPrice", value: "\(invoice.customerName!), \(invoice.discount!), \(invoice.orderID!), \(invoice.productID!), \(invoice.productName!), \(invoice.quantity!), \(invoice.salesperson!), \(invoice.shipperName!), \(invoice.unitPrice!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! InvoiceDetailViewController
...
extension InvoiceMasterViewController: EntitySetUpdaterDelegate {
