//
// CustomerDemographicDetailViewController.swift
// Employees
//
...
class CustomerDemographicDetailViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private let logger = Logger.shared(named: "CustomerDemographicMasterViewControllerLogger")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "updateEntity" {
            ...
            let detailViewController = dest.viewControllers[0] as! CustomerDemographicDetailViewController
    ...
    override func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 2:
            ...
            var masterViewController = destinationStoryBoard.instantiateViewController(withIdentifier: "CustomerMaster")
            func loadProperty(_ completionHandler: @escaping ([Customer]?, Error?) -> Void) {
                self.northwindEntities.loadProperty(CustomerDemographic.customers, into: self.entity) { error in
                    self.hideFioriLoadingIndicator()
                    if let error = error {
                        completionHandler(nil, error)
                        return
                    }
                    completionHandler(self.entity.customers, nil)
                }
            }
            (masterViewController as! CustomerMasterViewController).loadEntitiesBlock = loadProperty
            masterViewController.navigationItem.title = "Customers"
            (masterViewController as! CustomerMasterViewController).preventNavigationLoop = true
    ...
    private func cellForCustomerTypeID(tableView: UITableView, indexPath: IndexPath, currentEntity: CustomerDemographic, property: Property) -> UITableViewCell {
        var value = ""
        if let propertyValue = currentEntity.customerTypeID {
            value = "\(propertyValue)"
        }
        return CellCreationHelper.cellForProperty(tableView: tableView, indexPath: indexPath, entity: self.entity, property: property, value: value, editingIsAllowed: self.allowsEditableCells, changeHandler: { (newValue: String) -> Bool in
            ...
            if newValue.isEmpty {
                currentEntity.customerTypeID = nil
...
extension CustomerDemographicDetailViewController: EntityUpdaterDelegate {
